...
if (kurulus > 1296000000) kontrol = 'Hesap Durumu: Güvenilir.'
  moment.locale("tr");
    ...
    let yazı = (`${message.guild.name}'a Hoşgeldin`)
    .setAuthor(yazı, user.iconURL({dynamic:true}))
    .setColor("RANDOM")
...
if ((yeni.username).includes(tag) && !uye.roles.has(tagrol)) {
  try {
    ...
    await uye.send(`Tagımızı alarak sunucumuza hoşgeldin teşekkür ederiz ${yeni}.`);
...
if (!(yeni.username).includes(tag) && uye.roles.has(tagrol)) {
  try {
    ...
    await uye.send(`Tagımızı bıraktığın için üzgünüz ${yeni}, tekrardan aramıza dönmek istersen çekinme lütfen.`);
    let embed2 = new MessageEmbed()
    .setDescription(`${yeni} adlı üye tagımızı bırakarak aramızdan ayrıldı.`)
    .setColor("ffffff")
    await client.channels.cache.get(kanal).send(embed2);
  } catch(err) { console.error(err) };
};
});
//-----------------------OTO-TAG-----------------------\\ 
